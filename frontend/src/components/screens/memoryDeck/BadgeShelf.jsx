...
export default function BadgeShelf({ visitedExhibitIds = [] }) {
  const badges = evaluateBadges(visitedExhibitIds);
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-imperial-gold/20 text-imperial-gold border border-imperial-gold/40">
        ...
        {badges.map((badge) => (
          <div
            ...
            className={`p-3 rounded-xl border transition-all duration-300 flex flex-col items-center text-center ${
              badge.unlocked
                ? "bg-imperial-gold/15 border-imperial-gold/60 text-parchment shadow-gold-glow ring-1 ring-imperial-gold/40"
